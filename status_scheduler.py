...
# ❎ Encerra posição real

def fechar_posicao_real(par, tipo, quantidade):
    try:
        print("[FECHAMENTO] Enviando ordem de encerramento...", flush=True)
        oposto = "sell" if tipo == "buy" else "buy"
        position_side = "LONG" if tipo == "buy" else "SHORT"
        ...
        notificar_telegram(f"❎ ORDEM DE ENCERRAMENTO ENVIADA\nPar: {par}\nQtd: {quantidade}")
        print("[FECHAMENTO] Ordem de encerramento enviada!", flush=True)
        return ordem
